...
class PlaySuissePlayer:
    """Natively resolves and plays Play Suisse streams without relying on
    external binaries.
    """
    ...
    def resolve_and_play(self, handle, asset_id, title):
        ...
        try:
            ...
            try:
                ...
                xbmc.log(
                    "PlaySuissePlayer: Playback session creation failed, "
                    f"falling back: {e}",
                    xbmc.LOGWARNING,
                )
            ...
            err_str = str(e)
            xbmc.log(
                f"PlaySuissePlayer: Playback resolution failed: {err_str}",
                xbmc.LOGERROR,
            )
        ...
        # 4. Locate the HLS stream if not already obtained from playback
        # session
        if not hls_url:
        ...
        if not hls_url:
            xbmc.log(
                f"PlaySuissePlayer: No HLS stream found for asset {asset_id}",
                xbmc.LOGERROR,
            )
        ...
        if not helper.check_inputstream():
            xbmc.log(
                "PlaySuissePlayer: Unable to setup inputstream.adaptive",
                xbmc.LOGERROR,
            )
            return
